...
CREATE TABLE `companies` (
    ...
    `billing_model` ENUM('hourly','fixed') NULL DEFAULT NULL COMMENT 'Per-company billing model shown in attendance view: hourly=Hodinová sazba, fixed=Paušál, NULL=not set',
    `hourly_rate` DECIMAL(10,2) NULL DEFAULT NULL COMMENT 'Hourly rate in CZK shown in attendance view when billing_model=hourly; NULL when not set or when billing_model is fixed/Neurčeno',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
CREATE TABLE `maintenance_requests` (
    ...
    `source` ENUM('portal','whatsapp','phone','in_person','email') NOT NULL DEFAULT 'portal',
